...
export interface RadialScrollGalleryProps
   ...
   * Receives the current active index (based on scroll) and hovered index.
   */
  children: (activeIndex: number, hoveredIndex: number | null) => ReactNode[];
  /**
...
>(
  (
    ...
    const childrenNodes = useMemo(
      () => React.Children.toArray(children(activeIndex, hoveredIndex)),
      [children, activeIndex, hoveredIndex]
    );
      ...
      () => {
        ...
        if (!prefersReducedMotion) {
          ...
          gsap.to(containerRef.current, {
            rotation: direction === 'ltr' ? 360 : -360,
            ease: 'none',
            scrollTrigger: {
              trigger: pinRef.current,
              pin: true,
              ...
              onUpdate: (self) => {
                // Calculate which item is at the "top" (270 degrees)
                // Item angle = (i/count)*360
                // Current item position = (Item angle + container rotation) % 360
                // Target position = 270 (top of circle)
                const currentRotation = self.progress * (direction === 'ltr' ? 360 : -360);
                
                // We want to find i such that ( (i/count)*360 + currentRotation ) % 360 approx 270
                // i = ( (270 - currentRotation) % 360 / 360 ) * count
                const targetAngle = 270;
                let normalizedRotation = (targetAngle - currentRotation) % 360;
                if (normalizedRotation < 0) normalizedRotation += 360;
                
                const index = Math.round((normalizedRotation / 360) * childrenCount) % childrenCount;
